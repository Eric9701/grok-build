...
//!
//! **Trust storage**: `~/.atlas/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.atlas/`.
...
impl TrustStore {
    /// If `~/.atlas/trusted-plugins` does not exist, returns an empty store.
    ...
    /// Canonicalizes the path and appends it to `~/.atlas/trusted-plugins`.
    ...
    /// rewrites `~/.atlas/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.atlas/trusted-plugins`.
