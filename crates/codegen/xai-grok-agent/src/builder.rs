...
impl AgentBuilder {
    ...
    /// generic ACP). Suppresses prompt sections that require a human at the
    /// TUI and stamps `non_interactive` into the ask-user-question params, so
    /// an unanswered questionnaire returns no-operator text instead of
    /// "user declined". Atlas path guidance in `<user_guide>` remains because
    /// ACP / VS Code sessions need it too.
