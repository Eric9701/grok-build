...
/// → Bundled (injected `config.bundled_skill_dirs` + `~/.atlas/bundled`; lowest precedence).
///
...
fn scope_for_config_dir(dir: &Path, cwd: Option<&Path>, git_root: Option<&Path>) -> SkillScope {
    // Home-level dirs (e.g. ~/.atlas/, ~/.agents/, ~/.claude/) are User scope.
    if let Some(home) = xai_dirs::home_dir()
...
mod tests {
    ...
    fn dedupe_same_scope_same_basename_still_drops() {
        ...
        // (e.g. ~/.atlas/skills and ~/.agents/skills): first-seen wins.
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.grok/skills/japandi/SKILL.md",
