...
/// No auth required for the local atlas publish layout.
///
...
/// Each individual base also retries up to 3 times with exponential backoff (1s, 2s, 4s) on transient failures before falling through to the next base.
...
/// - `"internal"`: reads the channel pointer from configured CLI base URLs (atlas-server `/cli` by default).
