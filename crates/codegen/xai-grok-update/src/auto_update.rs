...
fn atlas_cli_base() -> String {
    crate::version::resolve_cli_base_urls(None, None)
        .into_iter()
        .next()
...
fn manual_install_cmd(channel: &str) -> String {
    ...
    if channel == "enterprise" {
        ...
        return if cfg!(windows) {
            format!("irm {base}/install-enterprise.ps1 | iex")
        } else {
            format!("curl -fsSL {base}/install-enterprise.sh | bash")
        };
    }
