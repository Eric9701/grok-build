...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    if crate::util::path_present_or_uncertain(&hook_root.join(".grok").join("hooks"))
        || crate::util::path_present_or_uncertain(
            &xai_grok_config::project_dir_in(hook_root).join("hooks"),
        )
    ...
    // Project AGENT dirs (`.atlas/agents` / `.grok/agents` / `.claude/agents`): an agents-only clone must still be gated
