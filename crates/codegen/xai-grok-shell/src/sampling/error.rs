...
pub const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free Atlas Build usage limit for now. Get SuperGrok for much higher limits, or try again later: https://grok.com/supergrok?referrer=grok-build";
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("xai-test"), || {
            ...
            assert!(
                ...
                "should suggest atlas logout when API key is available: {msg}"
            );
            assert!(
                msg.contains("/logout"),
