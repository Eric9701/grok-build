...
/// Add a new git or local-path marketplace source to `~/.atlas/config.toml`.
                ...
                return ActionOutcome {
                    ...
                    message: format!(
                        "{e}. Not a reachable git repository — to add it anyway (e.g. a \
                         VPN-gated host), run: atlas plugin marketplace add {url} --force"
                    ),
                    requires_reload: false,
                    requires_restart: false,
                };
...
/// Remove a marketplace source from `~/.atlas/config.toml` and uninstall all
