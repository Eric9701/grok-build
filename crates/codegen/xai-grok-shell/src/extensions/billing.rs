...
//!
//! Fetches the authenticated user's Atlas Build billing configuration (credit limit, usage, on-demand cap, billing period, history) from the backend.
...
/// Current billing configuration for Atlas Build coding credits. Carries the newer credits-config fields (`credit_usage_percent`, `current_period`).
...
/// Keeps history to a count + the most recent period so `~/.atlas/logs/unified.jsonl` stays useful without dumping unbounded period arrays.
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Billing data requires auth with grok.com. Run `atlas login` to authenticate.",
    )?;
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Auto top-up data requires auth with grok.com. Run `atlas login` to authenticate.",
    )?;
