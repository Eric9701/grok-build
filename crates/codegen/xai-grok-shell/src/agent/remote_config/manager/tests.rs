...
use super::super::{
    ...
    degraded_log_level, evaluate_models_commit, overlay_at_rest_secrets, resolve_prefetch_inputs_from_parts,
...
fn atlas_cache_scope(origin: &str) -> ModelsCacheScope {
    ModelsCacheScope {
        auth_method: CacheAuthMethod::Session,
...
fn managed_cache_keeps_api_key_and_info_model_enc_on_disk() {
    ...
    let scope = atlas_cache_scope(origin);
    cache.persist_catalog(&models, Some("etag-m"), &scope, Utc::now(), true);
...
fn managed_cache_with_plaintext_secrets_is_a_miss_and_is_deleted() {
    ...
    let scope = atlas_cache_scope(origin);
    cache.persist_catalog(&models, None, &scope, Utc::now(), true);
...
fn legacy_plaintext_api_key_cache_is_invalidated() {
    ...
    let scope = atlas_cache_scope(origin);
    cache.persist(&models, None, &scope, Utc::now());

    assert!(cache.load_fresh(&scope).is_none());
...
fn cache_file_is_owner_rw_only() {
    ...
    let tmp = tempfile::TempDir::new().unwrap();
    let cache = test_cache_manager(tmp.path());
    let scope = atlas_cache_scope("https://atlas.test/v1/models");
    cache.persist(
        &make_prefetched(&["grok-4.5"]),
        None,
        &scope,
