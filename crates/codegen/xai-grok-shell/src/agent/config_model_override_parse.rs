...
//!
//! Warnings are retained on `Config::config_warnings` and surfaced by `atlas inspect`.
...
/// Serialize-only: `atlas inspect --json` emits it, nothing deserializes it back.
...
pub(crate) fn log_config_warnings(warnings: &[ConfigWarning]) {
    ...
    if !warnings.is_empty() {
        tracing::warn!(
            ...
            "config: parsed with warnings; run `atlas inspect` for details"
        );
    }
}
