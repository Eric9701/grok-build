...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `atlas login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `atlas login`, set XAI_API_KEY, or add api_key to ~/.atlas/config.toml.";
...
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `atlas login` to authenticate.";
...
pub(crate) fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.atlas/auth.json".to_string())),
    )
}
...
pub(crate) fn grok_com_auth_method(
    ...
    let name = label.unwrap_or("Atlas");
...
mod tests {
    ...
    /// `~/.atlas/config.toml` skeleton from the bug report, walks it through
    ...
    //
    // `atlas login --legacy` produces a GrokAuth with `auth_mode: WebLogin`, `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL)
    ...
    fn grok_login_legacy_token_does_not_require_login() {
        ...
        // Construct a legacy-style token exactly as `atlas login --legacy` produces it
