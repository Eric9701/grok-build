use crate::auth::backend::{ActiveAuthBackend, AuthBackend, LoginRequest};
...
/// Reads `[auth] login_device_flow` from a config snapshot (shared with the proxy-URL read).
...
/// Device-flow precedence, highest first: CLI, env, config, remote feature flag, then device.
...
///
/// Precedence, highest first: the `--oauth`/`--device-auth` CLI flags, `GROK_LOGIN_DEVICE_FLOW` env, then `[auth] login_device_flow` config.
/// Below those, the `grok_build_login_device_flow` remote feature flag decides; **device** is the Atlas default.
...
pub(super) async fn run_auth_flow_steps(
    ...
    // `atlas login --devbox` uses run_devbox_login and is not gated here.
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `atlas login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
/// Carries no timeout of its own: the readiness-path caller imposes `STARTUP_AUTH_TIMEOUT`.
/// The leader's background re-mint runs uncapped (only the provider's ~300s ceiling).
pub(crate) async fn mint_session_noninteractive(
    ...
    if crate::auth::devbox_login::is_devbox_environment() {
        match crate::auth::devbox_login::mint_devbox_auth(auth_manager).await {
            Ok(new_auth) => {
                return Some(persist_or_use_minted(auth_manager, new_auth).await);
            }
            Err(e) => {
...
/// Unified `atlas login` handler for CLI entry points (tui, pager).
///
...
async fn run_cli_login_steps(
    ...
    // supports the device flow. Without this guard, `atlas login` on an
...
///
/// When `scope` is `None`, clears the default scope (same as `/logout` in the TUI).
/// When `Some`, removes only that scope entry.
...
mod tests {
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `atlas login` must use loopback, not device —
    ...
    /// CLI `atlas login` passes `on_stderr=None`; stderr must be inherited so sign-in URLs appear in real time.
