...
///
/// When `git_ref` is set, forces a clean checkout of that ref (same as the manual `create_from_worktree_sync` path used by `atlas -w --ref`).
...
mod tests {
    ...
    async fn checkout_persisted_head_stashes_dirty_worktree_state() {
        ...
        assert!(
            list_out.contains("atlas: pre-restore-code sess-dirty-wt"),
