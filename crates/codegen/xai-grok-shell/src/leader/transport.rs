...
//!   The leader's filesystem path is hashed into `\\.\pipe\atlas-leader-<hash>` so callers keep their path-based API.
//!
#[cfg(unix)]
...
mod windows_impl {
    ...
    /// Deterministic leaf name (`atlas-leader-<hash>`) for a filesystem path.
    ///
    ...
    fn pipe_leaf_name(path: &Path) -> std::ffi::OsString {
        ...
        std::ffi::OsString::from(format!("atlas-leader-{hash:016x}"))
    }
    ...
    mod tests {
        ...
        fn pipe_name_has_correct_prefix() {
            ...
            assert!(s.starts_with(r"\\.\pipe\atlas-leader-"), "got: {s}");
        }
        ...
        fn pipe_name_is_bounded() {
            ...
            // \\.\pipe\atlas-leader- (20 chars) + 16 hex chars = 36 total
