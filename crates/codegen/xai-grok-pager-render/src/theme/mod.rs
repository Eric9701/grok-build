...
pub enum ThemeKind {
    ...
    /// The `"auto"` string is stored on disk and in `app.current_ui.theme`, but only the resolved concrete kind lives in `cache::CURRENT`.
...
impl ThemeKind {
    ...
    ///
    /// TokyoNight uses blue-tinted backgrounds that lose their character when quantized to 256 or 16 colors.
    /// GrokNight uses neutral grays that survive quantization cleanly.
...
impl Theme {
    ...
    /// On 256-color terminals they are mapped to the nearest indexed palette entry.
    /// On 16-color terminals they map to ANSI names.
    ...
    ///
    /// On Windows applies a contrast boost so structural RGB survives the display gamma.
    /// At [`ColorLevel::Basic`] (or legacy ConHost below truecolor) we additionally pin chrome colors to ANSI-named entries.
    /// Without the pins every dark RGB collapses onto the same ANSI16 slot.
    /// Modern ConHost (Win10 1709+) lands on TrueColor via [`color_support::terminal_supports_truecolor`] and skips the overrides.
    pub fn current() -> Self {
        ...
        // The ANSI16 chrome fallback fires in two cases:
        //   1. Any terminal that only advertises 16-color support (e.g., `TERM=xterm`, `TERM=ansi`, or `GROK_FORCE_COLOR_LEVEL=basic`).
        //      Naive quantization there collapses every dark RGB onto `Color::Black`
        //   2. Legacy Windows ConHost below TrueColor, kept for parity with the glyph fallback path also gated on `is_legacy_windows_console()`.
        //
        // Both arms require `has_color()` so that `NO_COLOR` (which produces `ColorLevel::None`) keeps suppressing all SGR output
        // Without the explicit gate on the legacy-Windows arm, `ansi16_chrome_overrides` would repaint `Color::Reset` slots with named ANSI colors
        // That would partially defeat the user's opt-out on ConHost
        if level.has_color()
    ...
    /// Push structural colors further from `bg_base` so they survive Windows display gamma (especially non-HiDPI panels).
    /// The theme's native ~12-unit RGB steps collapse visually there.
    ///
    /// Per-field push amounts are tuned for ConHost specifically.
    /// Without colorimetric calibration it takes ~24-32 levels per channel before mid-gray distinctions read at all.
    ///
    /// The user-prompt block bg is the one asymmetric field: a dark step on a light canvas "weighs" far more than the light step on a dark canvas.
    /// We push much less in that direction to stay close to the theme's native RGB on light.
    ...
    /// Pin chrome and semantic-accent colors to ANSI-named entries so they survive 16-color quantization.
    /// `dark` flips polarity along two axes:
    ///
    /// 1. **Chrome (bg/borders/scrollbar/text).** ANSI16 has only two grays (DarkGray, and Gray, which is silver) plus Black and White.
    ///    Chrome pins to those four to preserve the bg/border hierarchy that the theme defines via subtle gray steps.
    /// 2. **Semantic accents (running/error/success/etc.).** Naive RGB-distance quantization collapses pastel theme hues onto the gray ramp.
    ///    An audit on groknight found 18 of 27 fields became DarkGray or silver, erasing every state signal.
    ///    We pin each semantic field to a hue-preserving ANSI16 slot, polarity-aware.
    ///    Bright variants (`Light*`, idx 9-15) go on a dark canvas; normal variants (idx 1-7, ~50% luminance) on a light canvas.
    ///    This drops sub-hue differentiation that ANSI16 cannot represent (e.g., teal and cyan both pin to `Cyan` / `LightCyan`).
    ///    In exchange error reads red, success reads green, etc.
    ///
    /// Applied both on legacy Windows ConHost and on any terminal that advertises only [`color_support::ColorLevel::Basic`].
    ///
    /// Surfaces that blend with the body canvas (sunken code blocks, scrollbar track, paste chip bg) pin to the theme polarity, not `Color::Reset`.
    /// `Reset` would defer to the user's terminal profile, which can disagree with the selected theme.
    /// GrokNight on a white terminal, for example, would show white "holes" through every sunken surface.
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // Pin to the theme's polarity, NOT Color::Reset
            // The truecolor "subtle sunken / code block" effect can't be replicated in 16-color
            // Using the theme polarity guarantees these blend cleanly with `bg_base` regardless of what the user's terminal canvas is set to
            bg_dark: canvas_bg,
            ...
            // The four-tier truecolor border hierarchy collapses onto three ANSI16 slots:
            //   - `prompt_border` (idle text-input frame) pins to `muted_fg`, not `dim_fg`
            //     DarkGray (ANSI 8) is tuned near-bg on many palettes, so a dim idle frame vanishes the same way table chrome did
            //   - `hover_border` (transient mouse-hover) pins to `DarkGray`, stable across both polarities so a hover band reads consistently
            //   - `selection_border` (sticky selection) pins to `muted_fg`, one tier louder than dim, drawing the eye without screaming
            //   - `prompt_border_active` (focused) pins to `high_contrast_fg`, maximum contrast so focus always pops
            ...
            // State signals (running / completed / error) and content categories (system / skill / etc.) pin to a hue that survives ANSI16
            // ANSI16 only has 6 chromatic slots (no orange, no teal, no violet), so several truecolor accents intentionally fold onto the same slot
            // The goal is "preserve the dominant hue family", not "preserve every sub-hue"
            //
            // Magenta family (assistant turn, mid-stream thinking, running indicator, context-overhead accent)
            // All four use a purple/violet hue in both built-in themes
...
///
/// `Theme::current()` quantizes to the terminal's color level.
/// On non-truecolor terminals `accent_user` may be `Color::Indexed` or a named ANSI variant.
/// OSC 12 accepts an RGB triple regardless of the terminal's normal SGR color depth.
/// We resolve every variant back to RGB via [`crate::render::color::resolve_to_rgb`].
/// Reset (when `NO_COLOR` is set) yields `None`: we skip emission entirely so the terminal keeps its profile-defined cursor color.
///
...
mod tests {
    ...
    fn ansi16_overrides_border_hierarchy_is_distinct() {
        ...
        // On dark canvas, idle prompt and selection share `Gray` (silver) so the frame survives palettes that tune ANSI 8 near-bg
        // `hover_border` stays `DarkGray`, `prompt_border_active` is `White`.
        // On light canvas, `prompt_border` / `selection_border` / `hover_border` all sit on `DarkGray`; focus is `Black`
        use ratatui::style::Color;
        let t_dark = Theme::groknight().ansi16_chrome_overrides(true);
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        // Regression test for oscura-midnight: its thumb (`ELEVATED`, summed RGB 55) was *darker* than its track (`HIGHLIGHT_LOW`, 62)
        // That made the scrollbar invisible
        // Follow mode makes this stricter: the main-chat scrollbar blends the thumb 40% toward the track (`views/agent.rs`)
        // The full-strength delta must be comfortably visible to survive that blend
        //
        // Requirements per theme, in native truecolor RGB:
        //   1. Polarity: thumb sits *away* from the canvas relative to the track, lighter on dark themes, darker on light themes.
        //   2. Magnitude: at least 30 summed-RGB units between thumb and track (the smallest shipping delta, tokyonight, is 34).
        use ratatui::style::Color;
    ...
    fn ansi16_quantize_without_override_collapses_groknight_backgrounds() {
        ...
        // Naive `Basic` quantization maps every dark GrokNight bg field to `Color::Black`, erasing the hierarchy
        // This test makes the motivation for `ansi16_chrome_overrides` explicit
        // If quantization later gains a "dark gray" mid-tone (e.g., via an ANSI24/ANSI32 level), this test starts failing
        // Revisit the override scope then
        use ratatui::style::Color;
    ...
    fn ansi16_overrides_gray_hierarchy_collapses_to_two_slots() {
        // Basic exposes only DarkGray and Gray as named greys (Black and White are reserved for canvas / max-contrast fg)
        // We split the dim/medium/bright triplet into two tiers:
        //   - `gray`, `gray_bright` pin to `muted_fg` (the higher-contrast slot: silver on Black, charcoal on White)
        //     Secondary text needs to stay readable
        //   - `gray_dim` pins to `dim_fg` (the lower-contrast slot: DarkGray on Black, silver on White)
        //     Used for genuinely faded chrome (modal frames, unselected `>` prompt indicator)
        // ANSI16 has only two grey slots, so a true three-tier hierarchy is impossible
        // Collapsing bright and medium onto the brighter slot keeps secondary text legible while still separating "dim" from "muted"
        use ratatui::style::Color;
        let t_dark = Theme::groknight().ansi16_chrome_overrides(true);
