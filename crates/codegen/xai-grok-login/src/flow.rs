...
/// Otherwise `GROK_LOGIN_DEVICE_FLOW` env, then `[auth] login_device_flow` config,
/// then the remote flag; device flow is the Atlas default.
/// Both transports run through `run_auth_flow_inner` so the external auth provider is tried first.
