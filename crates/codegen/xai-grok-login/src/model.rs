...
/// auth.json scope key for plain API key auth (desktop login, `atlas login --api-key`).
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from desktop login or `atlas login --api-key`)
    ApiKey,
}
...
/// Look up auth from the store by scope key. Legacy `WebLogin` tokens (from the pre-OIDC `atlas login --legacy` flow) are skipped. They are validated via a per-request DB lookup server-side, which fails at high volume.
