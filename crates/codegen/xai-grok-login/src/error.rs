...
#[non_exhaustive]
pub enum AuthError {
    #[error("Not logged in. Run `atlas login`.")]
    NotLoggedIn,

    /// The token expired and no refresh authority is available.
    #[error("Token expired. Run `atlas login` to re-authenticate.")]
    TokenExpiredNoRefresh,

    /// Server rejected the token (401) with no recovery path.
    #[error("Authentication rejected by server. Run `atlas login` to re-authenticate.")]
    ...
    #[error("{message} Run `atlas login` to sign in with the required team.")]
    PinnedTeamMismatch { message: String },

    /// The cached API-key session was rejected because API-key auth is disabled.
    #[error("API-key auth is disabled by your administrator. Run `atlas login` to authenticate.")]
    ApiKeyAuthDisabled,
...
impl RefreshTokenFailedReason {
    ...
    pub fn blocks_unattended_retry(self) -> bool {
        match self {
    ...
    pub fn user_message(self) -> Cow<'static, str> {
        match self {
            Self::RefreshTokenRejected => {
                "Your session has expired. Run `atlas login` to sign in again.".into()
            }
            Self::ClientRejected => {
                "Authentication is temporarily unavailable. Run `atlas login` if this persists."
                    .into()
            }
            Self::ProviderInteractiveRequired => provider_login_message(None),
            Self::Other => {
                "Authentication could not be refreshed. Run `atlas login` to sign in again.".into()
            }
        }
    }
}
...
pub fn provider_login_message(label: Option<&str>) -> Cow<'static, str> {
    match label {
        Some(label) => format!(
...
impl AuthError {
    ...
    pub fn transient_source(source: impl Into<Box<dyn std::error::Error + Send + Sync>>) -> Self {
    ...
    pub fn permanent(reason: RefreshTokenFailedReason) -> Self {
