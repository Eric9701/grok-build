...
/// auth.json scope key used by the pre-OIDC `atlas login --legacy` flow.
...
impl GrokComConfig {
    ...
    /// 3. Absent → enabled
    pub fn require_session_at_startup(&self) -> bool {
    ...
    /// The pin is fail-closed; explicit `atlas login --devbox` and `--api-key` bypass it.
