...
/// The settings modal renders a 1-line "Ask Atlas" tip footer at the bottom of the content area.
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        assert!(
            all_text.contains("Ask Atlas"),
            "[{fixture_label}] docs footer (`Ask Atlas`) must appear in the rendered modal:\n\
             {all_text}"
        );
        assert!(
