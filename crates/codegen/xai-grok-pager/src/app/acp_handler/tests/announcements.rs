#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.atlas` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.atlas` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.atlas` reads).
    #[test]
