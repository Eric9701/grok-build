...
pub(super) fn render(failure: &StartupFailure) -> String {
    ...
    let mut report = match &failure.reason {
        Reason::TimedOut { waited, timings } => {
            ...
            format!(
                "Couldn't start Atlas: startup timed out after {}.\n\n{explanation}",
...
impl NextStep {
    fn text(self) -> &'static str {
        match self {
            Self::Retry => "Start Atlas again.",
            Self::CheckNetworkThenRetry => "Check your network connection, then start Atlas again.",
            Self::RestartSharedLeader => {
                "Stop it with the command below, which also stops any other Grok \
                 session using it, then start Atlas again."
            }
        }
    }
