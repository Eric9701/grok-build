...
/// resolves the id to `PromptOutcome::AllowOnce`), then (b) reuses the existing `set_yolo_mode(true)` flow to flip the local YOLO state, drain any remaining queued permissions, persist `[ui] permission_mode = "always-approve"` to `~/.atlas/config.toml`, and fire the `x.ai/yolo_mode_changed` ACP notification.
