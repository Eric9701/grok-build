...
pub enum Command {
    /// Run Atlas without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration Atlas discovers for this directory
    Inspect {
        /// Emit machine-readable JSON output.
        #[arg(long)]
        json: bool,
    },
    /// Check terminal, clipboard, color, and input support without starting Atlas
    ...
    Login {
        ...
        #[arg(long, hide = true)]
        legacy: bool,
        /// Use Atlas OAuth (default issuer: atlas-server).
    ...
    Setup {
        /// Print the fetched configuration as JSON instead of installing it; writes nothing to ~/.atlas.
        #[arg(long)]
        json: bool,
    },
...
Examples:
  atlas wrap docker exec -it my-container bash
  atlas wrap kubectl exec -it my-pod -- bash

See ~/.atlas/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    Update {
        ...
        enterprise: bool,
        /// Internal: what spawned this `atlas update` (`user_command`, `auto_background`, `leader_converge`). Hidden.
        #[arg(long, hide = true)]
    ...
    /// Show what the Atlas home (~/.atlas) uses on disk
    ...
    /// Disabled when `[dashboard].enabled = false` in `~/.atlas/config.toml` or when the `GROK_AGENT_DASHBOARD=0` env var is set.
    Dashboard,
}
...
/// Targets a running leader process by PID (used by `atlas leader` / `atlas workspace`).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `atlas leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "atlas: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("atlas: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub enum AgentCmd {
    ...
    Stdio,
    /// Run the agent headlessly over the Atlas WebSocket relay
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.atlas/leader.sock`.
    ...
    /// Name it `~/.atlas/leader-*.sock` to keep `atlas leader list/kill` able to find it; any other location works but won't be auto-discovered
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// Default for all `atlas -p` runs is to wait (up to `--background-wait-timeout`) so eval harnesses see full task completion.
    ...
    /// To default plain `atlas` to minimal, set `[ui] screen_mode = "minimal"` in ~/.atlas/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.atlas/logs/sampling.jsonl.
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `atlas "fix the bug"` or `atlas --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    /// This is the `atlas "<prompt>"` launch form; the headless `-p`/`--single` path is handled separately.
