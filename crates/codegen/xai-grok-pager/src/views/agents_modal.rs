...
/// User-level vs project-level config files (`~/.atlas` vs `{cwd}/.grok`).
...
/// Union bundled personas with local `~/.atlas/personas` and `{cwd}/.grok/personas`.
///
...
/// Shared guard: canonical path under `~/.atlas/{subdir}` or `{cwd}/.grok/{subdir}`, not bundled.
