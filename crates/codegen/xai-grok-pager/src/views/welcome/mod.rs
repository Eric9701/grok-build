...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Atlas** VERSION+channel **Beta** (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | Atlas Beta [channel] (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Atlas Beta**  VERSION (left-aligned).
    HeroInline,
}
    ...
    let mut result = match params.auth_state {
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Atlas is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// Here the message is a multi-line block showing the workspace path and the warning that Atlas may run or modify contents in this directory.
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "Atlas may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
mod tests {
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(full.contains("Atlas"), "full badge: {full:?}");
        assert!(inline.contains("Atlas"), "inline badge: {inline:?}");
