...
/// Persisted dashboard configuration stored under `[dashboard]` in `~/.atlas/config.toml`. Lenient:
...
/// Load the full persisted dashboard from `~/.atlas/config.toml`. Returns `None` only when the file
...
pub fn write_persisted(p: &PersistedDashboard) -> std::io::Result<()> {
    let path = config_path()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no Atlas home"))?;
