...
impl ClipboardRecovery {
    ...
    fn legacy_fix(self) -> Option<&'static str> {
        match self {
            ...
            Self::UnverifiedOther => Some("atlas wrap or /minimal"),
...
fn clipboard_findings(
    ...
    match recovery {
        ...
        ClipboardRecovery::UnverifiedSsh => findings.push(manual_finding(
            ...
            "Atlas can't verify this clipboard route across the remote boundary",
            "When you copy, Atlas sends OSC 52 but can't confirm that the outer terminal accepted \
             it. Each copy is also saved to a backup file; the copy message shows the path. If \
             paste fails, run `atlas wrap ssh <host>` on your local computer or use `/minimal`. \
             For repeated SSH sessions, run `atlas doctor fix ssh-wrap` on your local computer.",
        )),
        ClipboardRecovery::UnverifiedContainer => findings.push(manual_finding(
            crate::diagnostics::CLIPBOARD_DELIVERY_UNVERIFIED_ID,
            FindingDisposition::Issue,
            "Atlas can't verify this clipboard route across the container boundary",
            "When you copy, Atlas sends OSC 52 but can't confirm that the outer terminal accepted \
             it. Each copy is also saved to a backup file; the copy message shows the path. If \
             paste fails, start the container command with local `atlas wrap <command>`, or use \
             `/minimal`.",
        )),
        ClipboardRecovery::UnverifiedOther => findings.push(manual_finding(
            crate::diagnostics::CLIPBOARD_DELIVERY_UNVERIFIED_ID,
            FindingDisposition::Issue,
            "Atlas can't verify this clipboard route",
            "Each copy is also saved to a backup file; the copy message shows the path. For a \
             remote or container command, use local `atlas wrap <command>`. You can also use \
        ...
        ClipboardRecovery::UnavailableSsh => findings.push(manual_finding(
            ...
            "When you copy, Atlas saves the text to the backup file shown in the copy message. To \
             copy directly, run `atlas wrap ssh <host>` on your local computer. For repeated SSH \
             sessions, run `atlas doctor fix ssh-wrap` there. You can also use `/copy <file>` or \
             `/minimal`.",
        )),
        ClipboardRecovery::UnavailableContainer => findings.push(manual_finding(
            ...
            FindingDisposition::Issue,
            "This clipboard route can't reach the target clipboard",
            "When you copy, Atlas saves the text to the backup file shown in the copy message. \
             Start the container command with local `atlas wrap <command>`, use `/copy <file>`, or \
             use `/minimal`.",
        )),
        ClipboardRecovery::UnavailableLocal => findings.push(manual_finding(
            ...
            FindingDisposition::Issue,
            "This clipboard route can't reach the target clipboard",
            "When you copy, Atlas saves the text to the backup file shown in the copy message. Use \
