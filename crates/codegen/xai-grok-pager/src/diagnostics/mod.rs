    ...
    DiagnosticFinding {
        ...
        note: Some(
            "Connect or select a microphone in your system sound settings. On Linux, install a \
             supported audio recorder if none was found on PATH. Then run `/doctor` or `atlas \
...
pub enum WarningCategory {
    ...
    /// The session runs over SSH without `atlas wrap` on the local end.
...
pub(crate) fn collect_startup_warnings_from(
    ...
    if ctx.brand == TerminalName::AppleTerminal && ctx.is_ssh {
        ...
        warning.note = Some(
            "Atlas also saves each copy to the backup file shown in the copy message. To copy \
             directly, run `atlas wrap ssh <host>` on your local computer or use a terminal that \
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        config_path: None,
        note: Some(format!(
            "Atlas is using the user profile. Compare `.atlas/sandbox.toml` with {}, then rename \
...
/// Pure SSH `atlas wrap` recommendation: suggests launching the session through `atlas wrap ssh <host>` on the user's local machine.
...
mod tests {
    ...
    fn sandbox_profile_conflict_warning_reports_conflicts() {
        ...
        assert!(w.note.as_deref().is_some_and(|note| {
            note.contains("rename or remove")
                && note.contains(".atlas/sandbox.toml")
    ...
    fn ssh_wrap_hint_suppressed_when_sink_active() {
        // An active OSC 52 sink means the session already runs under `atlas wrap`; adoption silences the hint by itself.
