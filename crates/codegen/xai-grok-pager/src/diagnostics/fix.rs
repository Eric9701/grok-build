...
const TMUX_SCANNER_CAVEAT: &str = "Atlas checks this file for direct global assignments of this option. Review sourced files, conditionals, plugins, and generated tmux setup yourself.";
...
const FIX_REGISTRY: &[FixSpec] = &[
    ...
    FixSpec {
        ...
        command: "atlas doctor fix terminal.tmux-clipboard",
    ...
    FixSpec {
        ...
        command: "atlas doctor fix terminal.dcs-passthrough",
    ...
    FixSpec {
        ...
        command: "atlas doctor fix terminal.tmux-extended-keys",
    ...
    FixSpec {
        ...
        command: "atlas doctor fix terminal.tmux-truecolor",
...
pub(crate) fn human_fix_command(id: DiagnosticId) -> Option<String> {
    fix_spec(id).map(|spec| format!("atlas doctor fix {}", spec.handle))
}
...
pub(crate) fn format_fix_preview(plan: &FixPlan) -> String {
    ...
    match &change.backup_path_hint {
        Some(path) => {
            let _ = writeln!(
                output,
                "\nBackup will be saved to: {}\nIf that file exists, Atlas will choose a unique name.",
                preview_path(path)
            );
        }
    ...
    match &plan.payload {
        FixPayload::SshWrap(_) => {
            output.push_str(
                "\nWhat this changes:\n  In new interactive shells, `ssh ...` runs as `atlas wrap ssh ...`.\n",
            );
            let _ = writeln!(
                output,
...
fn plan_ssh_wrap(
    ...
    Ok(FixPlan {
        ...
        caveats: vec![
            ...
            "`atlas wrap` starts the SSH process directly, so the alias does not loop.",
            "Atlas checks this file for direct SSH aliases and functions. Review sourced files, plugins, and generated shell setup yourself.",
        ],
