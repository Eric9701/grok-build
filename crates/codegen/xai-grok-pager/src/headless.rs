...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `atlas login` to authenticate \
         (or `atlas login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         atlas login --device-code\n\n\
         Alternatively, set the XAI_API_KEY environment variable \
         or run `atlas login` on a machine with a browser."
            .to_string()
    }
}
