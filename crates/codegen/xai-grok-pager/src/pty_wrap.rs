//! Local PTY wrapper: the engine behind `atlas wrap` (see [`crate::wrap_cmd`]).
//!
...
/// Run an arbitrary command inside a local PTY with OSC 52 output filtering. This is the engine behind `atlas wrap`:
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // intentionally detached: `atlas wrap` is short-lived and exits with the child.
