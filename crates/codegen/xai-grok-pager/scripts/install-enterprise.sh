#!/bin/bash
#
# Atlas CLI installer (enterprise channel) — atlas-server /cli/install-enterprise.sh
#
...
#   curl -fsSL http://10.218.220.237:22255/atlas/cli/install-enterprise.sh | bash -s 0.2.120
#
...
BASE_URL="${GROK_CLI_BASE_URL:-${GROK_ATLAS_SERVER:-http://10.218.220.237:22255}/atlas/cli}"
BASE_URL="${BASE_URL%/}"
...
if [ "$os" = "windows" ]; then
    ...
    chmod +x "$binary_path"
    ln -sf "$binary_path" "$BIN_DIR/atlas"
