...
Project rules let you configure Atlas per project or directory. By placing an AGENTS.md file in your repository, you can set coding conventions, build instructions, style guides, and any other instructions that Atlas should follow when working in that codebase.

---

## What Are Project Rules?

Project rules are Markdown files that Atlas reads and adds to its context. Atlas follows their content for every interaction in that tree.

This is the primary mechanism for teaching Atlas about your project's conventions, so you need not restate them each session.

---

## Supported File Names

Atlas checks for these filenames (in this order) within each directory:
...
Atlas loads every matching file in a directory, so a folder that contains both `AGENTS.md` and `CLAUDE.md` contributes both. On case-insensitive filesystems, names that resolve to the same file (such as `Agents.md` and `AGENTS.md`) are deduplicated and counted once. `Claude.md`, `CLAUDE.md`, and `CLAUDE.local.md` are supported for compatibility with Claude Code workflows. When Claude compatibility is enabled (the default), Atlas also scans your home-level `~/.claude/` directory for these filenames and, at each directory level, checks `.claude/CLAUDE.md` and `.claude/CLAUDE.local.md` -- the locations Claude Code uses for project memory. With Cursor compatibility enabled, the home-level `~/.cursor/` directory is scanned the same way.

### Rules Directories

In addition to AGENTS.md files, Atlas scans for `*.md` files in rules directories at each level (`<dir>`) from the repo root to the current working directory:
...
Atlas also scans home-level rules, regardless of where it starts. These roots are already vendor-specific, so rules live directly under `rules/`:

| Location | Notes |
|----------|-------|
| `$GROK_HOME/rules/` (default `~/.atlas/rules/`) | Always scanned; applies to all projects |
...
Atlas scans for project rules in this order:
...
When Atlas runs in `~/projects/my-app/src/components/`, it loads all three files. The instructions accumulate, so Atlas sees all of them.

### Deeper Files Take Precedence

Atlas orders the files from the repo root to the current working directory, so files in deeper directories appear later in its context and take precedence when instructions conflict. In the example above, if the root says "Use styled-components" but `components/AGENTS.md` says "Use CSS modules", the CSS modules instruction wins because it appears later.

### Auto-Loading Behavior

- Atlas loads the files from the repo root to the current working directory automatically at session start.
- When Atlas reads, lists, or edits files in directories outside that initial set, it detects any project instruction files there, notes their paths, and reads them when they apply to the task.
...
```bash
atlas --rules "Always use TypeScript. Prefer functional components."
```

Atlas appends this text to the session's system prompt. Use it for session-specific customization.

To replace the system prompt entirely, pass `--system-prompt-override` (alias `--system-prompt`). Atlas uses the text verbatim and skips both the default system prompt and `--rules`. (Text passed with `--rules`, by contrast, is wrapped in a `<human_rules>` block and appended to the default prompt.)

---

## File Size

Atlas loads each project instruction file in full; there is no character cap and no truncation. Even so, keep instructions concise and focused. Shorter, specific rules are easier for Atlas to follow than long ones, and every file you load consumes context.
...
As top-level instruction files, Atlas discovers only the recognized filenames listed under [Supported File Names](#supported-file-names) — not custom names such as `AGENTS.local.md` or `notes.md`. (Inside a rules directory such as `.atlas/rules/`, every `*.md` file is loaded regardless of name.)
...
Use `atlas inspect` to see all loaded project instructions:

```bash
atlas inspect
```

This shows each project instruction file it finds, with its path and approximate token count. Use it to confirm Atlas picks up your rules.
