...
Atlas Build runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in Atlas to check the current session and see available fixes. If
Atlas cannot start, run `atlas doctor` in your shell. Use `atlas doctor --json`
...
`atlas doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `atlas doctor fix dcs-passthrough --yes`.
...
to guess if that directory is unavailable or unsafe. Atlas preserves the file's
...
Atlas deliberately does **not** run `tmux source-file` or change the live tmux
...
Atlas detects these terminal emulators from environment variables:
...
- Inside tmux, variables that identify the outer terminal may not reach Atlas.
...
Inside tmux there are two separate questions: what color Atlas emits, and what
...
Atlas writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when Atlas runs inside tmux.
...
terminal focused. Older compositors may require Atlas to remain focused until
the copy message appears. Atlas shows a startup warning when this applies; run
...
Atlas emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `GROK_CLIPBOARD_NO_OSC52=1` before starting Atlas to
...
A remote Atlas process normally cannot read the local terminal's selection. Use
...
When Atlas cannot identify the outer terminal over SSH, it predicts that OSC 52
...
clipboard. Each copy is still saved to a backup file (`~/.atlas/last-copy.txt` by
...
through `atlas wrap`, for example `atlas wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `atlas wrap`, Atlas shows the one-time tip
...
For repeated SSH use, Doctor offers `atlas doctor fix ssh-wrap`. It also shows
...
> **Warning**: `atlas wrap` is experimental and may not work in every setup.
...
Zellij and tmux control mode can limit the alternate screen. Atlas normally uses
inline mode in those environments. Run `/doctor` to see the detected condition.
You can configure `[terminal] alt_screen` in `~/.atlas/pager.toml`, or run
`atlas --no-alt-screen` to confirm inline mode works.

### Zellij keybindings interfere with Atlas

Zellij can intercept Ctrl/Alt keys before they reach Atlas. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach Atlas, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in Atlas.
...
keys. Atlas therefore does not negotiate the protocol there, and Shift+Enter can
...
If Atlas stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, Atlas stops capture and shows
...
to the terminal hosting Atlas. Open **System Settings → Privacy & Security →
...
Run `atlas doctor`, or run `/doctor` while voice mode is on. The **Voice** section
shows the microphone Atlas would use. If no input device is available, Doctor
...
terminals, Terminal.app, Konsole, mlterm, and others). Atlas Build therefore
**does not** reorder RTL by default.

If Arabic or Persian in **scrollback** (or list content) reads backwards,
enable app-side reordering in `~/.atlas/pager.toml` (or project config):
