...
Atlas reads settings from config files, environment variables, and CLI flags. This page covers the common options. The field list for `config.toml`, `managed_config.toml`, and `requirements.toml` is [26-config-reference.md](26-config-reference.md) (extracted to `~/.atlas/docs/user-guide/` on launch).
...
5. **config.toml** (`~/.atlas/config.toml`)
...
A harness or ACP client that launches `atlas agent stdio` can inject settings without writing a `config.toml` or relocating `$GROK_HOME`:
...
Location: `~/.atlas/config.toml`. If the file is missing, Atlas uses its built-in defaults, so you only need to set the values you want to override.
...
You can also flip it from the settings pane (`/settings` → **Disable vim input mode**); Atlas writes your choice to `[ui] simple_mode`. `simple_mode` and `vim_mode` are independent — one governs the prompt editor, the other governs scrollback navigation. See [Keyboard Shortcuts](03-keyboard-shortcuts.md) for the full binding reference.
...
Toggle it at runtime with `/vim-mode`, or from `/settings` → **Vim scrollback navigation**. Atlas writes the change to `[ui] vim_mode` immediately and applies it to every future pager session, including new agents and subagents in the same process. There's no per-session override — `config.toml` is the source of truth on next launch. `vim_mode` is independent of `simple_mode`.

#### Screen mode

`[ui] screen_mode` is the **default render mode** for plain `atlas` launches. Set it from `/settings` → **Default screen mode** (restart required) or edit `config.toml` by hand — both write the file. CLI flags (`--minimal` / `--fullscreen`) and slash commands (`/minimal` / `/fullscreen`) are session-scoped and do **not** write this key; after a slash switch, the reverse command returns you for that session only.
...
Priority for `[mcp_servers]` and `[plugins]`: `.atlas/config.toml` (current dir) > `<repo-root>/.atlas/config.toml` > `~/.atlas/config.toml`. `[permission]` rules aren't overridden by priority — they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).
...
Background workflows — the `workflow` tool, named `.atlas/workflows/*.rhai` scripts, `/deep-research`, and `/workflow` launches — are **on by default**. Disable with config, env, or remote settings.
...
Project workflows are discovered from `<repo-root>/.atlas/workflows/`; user workflows from `~/.atlas/workflows/`. Discovery and invocation key off the script's `meta.name`, so keep each filename aligned with its `meta.name`. Built-ins win over project names, and project names win over user names, so keep names unique across scopes.
...
`atlas inspect` reports cells that still need session-start resolution as `?` until a value is available; cells with an explicit env or TOML value use that value. Affected discovery entries report `compatibilityStatus: "unresolved"` in JSON and `[compat unresolved]` in human output.
...
`[hints]` holds small persisted UI preferences: remembered answers and modal layout. Atlas writes these for you as you use the TUI, but you can edit or delete them by hand; removing a key restores the default.

`[hints]` is read from the **effective config merge**, with the usual precedence: system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`, higher layers winning. The TUI only ever **writes** these to your user `~/.atlas/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "atlas"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `atlas`. |
...
| Atlas Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

With `method = "auto"`, Atlas detects the terminal brand and picks the best protocol. Set `method` explicitly to override that.
...
command = "terminal-notifier -title 'Atlas' -message '$GROK_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$GROK_MESSAGE' ntfy.sh/my-atlas-alerts"
...
Set these only to point telemetry at your own infrastructure or to switch parts off. The built-in endpoint and credentials are managed by Atlas — leave them unset to use the defaults.
...
- `required_minimum_version` (`GROK_REQUIRED_MINIMUM_VERSION`) and
  ...
  the user to install an approved version. `atlas update` and `atlas --version` keep
...
- An explicit `atlas update --version X` is allowed above the ceiling, to recover
...
Location: `~/.atlas/pager.toml`. This controls the TUI's look and feel. Changes apply on restart.
...
| `~/.atlas/logs/` | Internal log files (e.g. `unified.jsonl`, MCP server logs) |
...
Some settings can be set per-project by placing files in `.atlas/` inside your repository:
