...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.atlas/agents/` or `~/.atlas/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.atlas/personas/`; applied during subagent resolution |
...
model = "grok-build"
prompt_file = ".atlas/prompts/researcher.md"
```
