...
Sessions are identified by a unique session ID (a UUIDv7 when Grok generates it; a client may supply its own ID with `-s`) and stored on disk under `~/.atlas/sessions/`. Set `GROK_HOME` to override the base directory; when it is unset, Grok uses `~/.atlas`.
...
`grok du` (alias: `grok disk-usage`) reports what the grok home (`~/.atlas`) uses on disk. It lists each top-level directory, largest first, then each worktree with its size, type, age, label, and path. Worktrees the registry does not track appear as `untracked`. Pass `--json` for the same report as machine-readable output.
