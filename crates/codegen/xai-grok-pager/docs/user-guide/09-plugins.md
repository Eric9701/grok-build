...
A plugin bundles skills, slash commands, agents, hooks, and MCP servers into one installable unit. You get plugins from a marketplace, install the ones you want, and Atlas loads what they add. To build and share your own, see [Create your own marketplace](#create-your-own-marketplace).
...
1. **Add the marketplace** so Atlas can show what it offers. Nothing installs yet.
...
```bash
atlas plugin marketplace add my-org/team-plugins                  # GitHub shorthand (owner/repo)
atlas plugin marketplace add https://gitlab.com/acme/plugins.git  # any git host, include https:// and .git
atlas plugin marketplace add ./my-marketplace                     # a local folder
```

List, refresh, and remove sources with `atlas plugin marketplace list`, `atlas plugin marketplace update [<name>]`, and `atlas plugin marketplace remove <url>`.
...
```bash
atlas plugin install deploy-tools --trust
```
...
Run `atlas plugin install <source>` without `--trust` and Atlas shows the source, warns that installing activates the plugin's hooks, MCP servers, and skills, then stops. Add `--trust` to go ahead. Only install plugins from sources you trust (see [Trust and security](#trust-and-security)).

A plugin's skills appear in the slash menu. When a skill name is ambiguous, Atlas shows the qualified form prefixed by the plugin name, for example `/deploy-tools:release`. To pick up a newly installed plugin, press `r` in the Plugins tab or start a new session.
...
```bash
atlas plugin list [--json] [--available]   # installed plugins (--available requires --json)
atlas plugin uninstall <name> [--confirm] [--keep-data]   # aliases: rm, remove
atlas plugin update [<name>]               # omit the name to update every plugin
atlas plugin enable <name>
atlas plugin disable <name>
atlas plugin details <name>                # show the plugin's component inventory
```
...
Set these in `~/.atlas/config.toml`:
...
Plugins are off by default, so list one in `enabled` to turn it on, or in `disabled` to discover it but skip loading it. Each entry is a plain plugin name (from `atlas plugin list`) or a full ID (`<scope>/<hash>/<name>`).

To hide the plugins and hooks interface entirely, set `disable_plugins = true` in `~/.atlas/pager.toml`.
...
Enabled plugins require trust to load skills, commands, hooks, MCP servers, and LSP servers. Untrusted plugin agents remain listed with frontmatter only. Atlas trusts plugins in `~/.atlas/plugins/` automatically; project plugins in `.atlas/plugins/` require trust. Install with `--trust` to grant it:

```bash
atlas plugin install <source> --trust
```

Trusted plugin `.mcp.json` servers attach to the session like other MCP config, and child agents inherit them. Plugin agents (`plugin-name:agent-name`) use the parent session's MCP servers by default, the same as user agents under `~/.atlas/agents/`; restrict that with the `mcpInheritance` frontmatter (see [Subagents](16-subagents.md#mcp-inheritance)). For safety, plugin agent frontmatter cannot declare `mcpServers` or hooks, or set `permissionMode: bypassPermissions`.
...
3. **List the plugins in `.atlas-plugin/marketplace.json`.** This is the index Atlas reads.
...
my-org-plugins/
  .atlas-plugin/
    marketplace.json      # the index Atlas reads (required)
...
Atlas reads the index from `.atlas-plugin/marketplace.json`. It also accepts `.atlas-plugin/plugin.json` and the `.claude-plugin/` equivalents.
...
Validate a plugin before publishing with `atlas plugin validate [<path>]`, and tag a release from the manifest version with `atlas plugin tag [<path>] [--push]`. Then point people at the repository. They add it once and install the plugins they want:

```bash
atlas plugin marketplace add my-org/my-org-plugins   # GitHub shorthand, a git URL, or a local path
atlas plugin install gdrive --trust
```
...
# Plugins stay off until enabled. List plugin names (from `atlas plugin list`)
# or full IDs (`<scope>/<hash>/<name>`).
[plugins]
enabled = ["gdrive"]
```

For a hands-off install with no per-person step, also place the plugin's files where Atlas discovers and trusts them automatically: `~/.atlas/plugins/`, or a directory your device-management tool manages that you point to with `[plugins].paths`. Then enable them with `[plugins].enabled`.
...
Marketplaces distribute Atlas content: skills, commands, agents, hooks, and MCP server configurations. They do not install a program onto a machine. A skill or MCP server that runs a helper binary (for example a custom sign-in tool) still needs that binary delivered separately, bundled with your deployment or pushed through your device-management tool.

---

## Troubleshooting

**A plugin you installed isn't showing up.** Plugins are off until enabled. Check `atlas plugin list`, then add the plugin's name or ID to `[plugins].enabled`, or press `Space` on it in the Plugins tab. Reload with `r` in the Plugins tab or start a new session.

**A plugin's skills, hooks, or MCP servers don't load.** They stay inactive until the plugin is trusted. Reinstall with `--trust`, or place the plugin under `~/.atlas/plugins/` (auto-trusted). See [Trust and security](#trust-and-security).
...
**See exactly what loaded.** Run `atlas inspect` (add `--json` for machine-readable output) to list every discovered plugin and the skills, agents, hooks, and MCP servers it provides, each labeled with its `plugin: <name>` source.
...
An optional `plugin.json` manifest can override paths or add metadata; without one, Atlas discovers components from these standard directories. For example, a `team-tools` plugin might bundle a deploy skill, a code-review agent, pre-commit hooks, and a Linear MCP server, installed together in one step.
...
Atlas sets these and overrides any same-named value in the hook's `env` map (the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases are set too). See the [Hooks guide](10-hooks.md) for every variable passed to hooks.
