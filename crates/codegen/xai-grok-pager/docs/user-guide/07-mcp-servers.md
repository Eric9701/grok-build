...
MCP (Model Context Protocol) servers extend Atlas with external tool integrations. They let Atlas interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to Atlas over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside Atlas's built-in tools. The model can discover and call these tools during a session.
...
MCP servers are configured in `~/.atlas/config.toml` under `[mcp_servers.<name>]` sections.
...
Atlas spawns a local process and communicates over stdin/stdout:
...
>
> - env `GROK_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; Atlas-native
>   wins if both set; Claude-style name, but we bound by **bytes** not tokens)
> - `config.toml` — user-level (`~/.atlas/config.toml`) **or repo-level**
>   (`.atlas/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.atlas/config.toml` >
...
atlas mcp list
atlas mcp list --json          # Machine-readable output

# Add a stdio server. Everything after -- is the server command, so flags
# like -y reach the server instead of being parsed by atlas.
atlas mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir
...
atlas mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
atlas mcp remove github

# Enable or disable a local/TOML (or compat-sourced) server
atlas mcp enable github
atlas mcp disable github

# Diagnose a server's configuration and connectivity
atlas mcp doctor               # Check every configured server
atlas mcp doctor github        # Check one server
atlas mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `atlas mcp add` writes to `~/.atlas/config.toml` (`--scope user`). Use `--scope project` to write to `.atlas/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `atlas mcp list` shows servers from both scopes, marking project-scoped ones with `(project)` and disabled ones with `(disabled)`.

`atlas mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.

`atlas mcp enable` / `disable` persist the personal on/off state to user `~/.atlas/config.toml` (`disabled_mcp_servers`, and `[mcp_servers.<name>].enabled` when that entry exists). Scope:

- **Known names:** user/project Atlas TOML, names already on the disabled list, compat sources (`.mcp.json`, Claude, Cursor), and **plugin** MCP servers (same discovery as doctor/`/mcps`).
...
MCP servers can be configured per-project by placing a `.atlas/config.toml` in your repository:
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. Atlas handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers Atlas's own OAuth client with the provider.

Atlas walks from the current directory up to the git repo root, loading `.atlas/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.atlas/config.toml` | All projects | Lowest |
| `<repo-root>/.atlas/config.toml` | This repository | Medium |
| `<cwd>/.atlas/config.toml` | Current directory | Highest |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Atlas reads most other config sections only from `~/.atlas/config.toml`.
...
You can enable or disable MCP servers without restarting Atlas (TUI `/mcps` or CLI — see [CLI Management](#cli-management)).
...
Atlas loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native Atlas config | `~/.atlas/config.toml`, `.atlas/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.atlas/config.toml` or the corresponding environment variable (`GROK_CURSOR_MCPS_ENABLED`, `GROK_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `atlas inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).

---

## MCP OAuth

For MCP servers that require OAuth authentication, Atlas handles the credential flow automatically. When an MCP server requests OAuth credentials, Atlas opens a browser-based authorization flow and stores the resulting tokens for future use.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). Atlas expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
1. The parent session actually connected the server (see Extensions / `atlas inspect`)
...
For stdio servers, Atlas captures the process's standard error to `~/.atlas/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.atlas/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `atlas inspect` to see all loaded MCP servers and their sources:

```bash
atlas inspect          # Human-readable
atlas inspect --json   # Machine-readable
```

### Debug Logging

```bash
RUST_LOG=debug GROK_LOG_FILE=/tmp/atlas.log atlas
