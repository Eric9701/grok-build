...
Atlas asks before doing anything risky — and can plan before it codes.

## Permissions

When Atlas wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: Atlas explores the
...
`s` requests changes — Atlas iterates until you're happy, then implements.
...
background — Atlas keeps working and you're notified when it finishes
