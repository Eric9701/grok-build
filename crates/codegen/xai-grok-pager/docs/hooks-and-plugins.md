...
Atlas Build supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.atlas/hooks/`
- **Project hooks** — from `.atlas/hooks/` in your repo
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.atlas/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `atlas plugin marketplace add <source>`)
...
Sources land in `~/.atlas/config.toml`:
...
Or in `~/.atlas/settings.json` / `~/.claude/settings.json`:
