...
Hooks let you run custom scripts or HTTP requests at key moments during a Atlas session, for example before or after a tool runs, when a session starts or ends, or when the agent sends a notification.
...
**Trusting a project**: The first time you open a project with hooks, open the hooks modal (`Ctrl+L` outside the VS Code family, or `/hooks` on any terminal) or run `/hooks-trust`. This is the same folder-trust gate as `--trust`, recorded in `~/.atlas/trusted_folders.toml`. Trust prevents untrusted repos from running arbitrary code.
...
**Tool name aliases**: Claude-style names like `Bash`, `Edit`, `Read` automatically match Atlas's internal names (`run_terminal_cmd`, `search_replace`, `read_file`).
...
The `hook_event_name` (snake_case key) carries Claude's PascalCase value; `hookEventName` (camelCase key) carries atlas's snake_case value.
...
Atlas injects the following variables into every hook process:
...
{
  "type": "command",
  "command": "${HOME}/.config/atlas-hooks/check.sh"
}
```

Lookup order for each reference:
1. The handler's own `env` map.
2. The current process environment (the env Atlas itself sees).
...
`GROK_SESSION_ID`) are not taken from the Atlas process environment at
...
- **`The argument '/.claude/hooks/….ps1' to the -File parameter does not exist`?** PowerShell treated `$CLAUDE_PROJECT_DIR` as empty. Atlas rewrites it to `$env:CLAUDE_PROJECT_DIR` unless `GROK_SHELL=cmd`.
