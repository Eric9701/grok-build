...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        for (i, proto) in protos.into_iter().enumerate() {
            ...
            let dep_out = tmp.join(format!(
                "xai-proto-deps-{}-{i}.d",
                std::process::id()
            ));
            let desc_out = tmp.join(format!(
                "xai-proto-desc-{}-{i}.bin",
                std::process::id()
            ));
            ...
            command
                .arg(format!(
                    "--dependency_out={}",
                    dep_out.to_str().context("dep_out path not UTF-8")?
                ))
                .arg(format!(
                    "--descriptor_set_out={}",
                    desc_out.to_str().context("desc_out path not UTF-8")?
                ));
            ...
            if !status.success() {
                let _ = fs::remove_file(&dep_out);
                let _ = fs::remove_file(&desc_out);
                return Err(anyhow::anyhow!("protoc command failed"));
            }

            let output = fs::read_to_string(&dep_out)
                .with_context(|| format!("failed to read {}", dep_out.display()))?;
            let _ = fs::remove_file(&dep_out);
            let _ = fs::remove_file(&desc_out);

            let mut lines = output.lines();
            let first_line = lines.next().context("protoc dependency output is empty")?;
            // Makefile dep format: "<descriptor_set_out>: <deps...>"
            // On Windows the target path contains a drive letter (`C:\...`), so
            // split on ": " (colon + space), not the first colon alone.
            let rem = first_line
                .split_once(": ")
                .or_else(|| first_line.rsplit_once(':'))
                .map(|(_, rest)| rest)
                .with_context(|| {
                    format!("protoc dependency output missing ':': {output:?}")
                })?;
