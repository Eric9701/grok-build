...
- **`/user-work-profile`** — 用 Atlas Task Report 做 **过程证据画像**（五维：流程纪律 / 并行效率 / 领域深度 / 工程效率 / 工具熟练；可出 L1–L4）。单人或多人。要有 Admin API；不是绩效考核产品。
...
## Enterprise Role Executors (atlas-sdd)
...
**Done when:** this session already ran `grill-with-docs`, or `CONTEXT.md` already records *this* piece of work.

**If not done:** tell the user to type `/grill-with-docs` and wait. Interview in **this** parent session (rounds, frontier). If the Skill tool happens to be available, you may load `grill-with-docs` by name (`skill=grill-with-docs`; it will not appear in the listing). Do not spawn any Role yet.

### Gate — Requirements (Role 1)

**Done when:** `documents/requirements-analyst/` has the artifact for this work (Bridge Sync from grill / CONTEXT / ADRs).
...
**Done when:** Role 4 has returned and the code exists.

**If not done:** spawn `atlas-sdd:4-software-engineer-agent` (`run_in_background: false`) only after detailed design exists. Prompt it to invoke `tdd` (red-green slices). It must not invoke whole `/implement`.

### Gate — Close

**Done when:** this parent session has run `/code-review` on the implementation diff (Standards + Spec). Under **enterprise-strict**, Role 6 has also written `documents/test-cases/`.

**If not done:** invoke `code-review` here (Skill tool or `/code-review`). Then spawn `atlas-sdd:6-qa-engineer-agent` when enterprise-strict still lacks test-case docs. Role 6 writes cases; it does not run `/code-review`.

Affinity (which skill a Role invokes): plugin `spec/role-skill-affinity.md`. Formal handoff stays the **Documents Contract** (`documents/…`).
