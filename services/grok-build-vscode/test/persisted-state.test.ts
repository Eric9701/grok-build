...
const installFile = `${DIR}/${DISK_KEYS["atlas.installId"]}`;
...
describe("PersistedState", () => {
  it("delegates keys it does not own straight to globalState", async () => {
    const { state, memento, fs } = make((_f, m) => m.store.set("atlas.cliUpdateExtVersion", "3.1.0"));
    expect(state.get<string>("atlas.cliUpdateExtVersion")).toBe("3.1.0");
    await state.update("atlas.implicitChipHidden", true);
    expect(memento.store.get("atlas.implicitChipHidden")).toBe(true);
  ...
  it("migrates existing globalState into the state directory on first load", async () => {
    ...
    const { state, fs } = make((_f, m) => m.store.set("atlas.sessionMeta", overrides));
    // Readable immediately — the migration write must not gate the read.
    expect(state.get("atlas.sessionMeta", {})).toEqual(overrides);
    await state.flush();
  ...
  it("PRESERVES an existing install id rather than minting a new one", async () => {
    ...
    const { state, fs } = make((_f, m) => m.store.set("atlas.installId", "abc-123"));
    expect(state.get<string>("atlas.installId")).toBe("abc-123");
    await state.flush();
  ...
  it("prefers the file over globalState once it exists", () => {
    const { state } = make((f, m) => {
      ...
      m.store.set("atlas.sessionMeta", { s1: { customName: "stale shadow" } });
    });
    expect(state.get<Record<string, { customName: string }>>("atlas.sessionMeta", {}).s1.customName).toBe("from disk");
  });
  ...
  it("hydrates globalState from a pre-existing disk value", () => {
    ...
    expect(memento.store.get("atlas.sessionMeta")).toEqual({ s1: { customName: "from disk" } });
    expect(state.get("atlas.sessionMeta", {})).toEqual({ s1: { customName: "from disk" } });
  });
  ...
  it("refreshes a synchronous read when another client changes the file", () => {
    ...
    expect(state.get<Record<string, { customName: string }>>("atlas.sessionMeta", {}).second.customName).toBe("two");
  });
  ...
  it("merges a stale record snapshot instead of erasing another client's entry", async () => {
    ...
    migratedMemento.store.set("atlas.sessionMeta", { migrated: { customName: "kept" } });
    const migrated = new PersistedState(migratedMemento, DIR, fs);
    await migrated.flush();

    await early.update("atlas.sessionMeta", { local: { customName: "also kept" } });
  ...
  it("still deletes what the writer deleted, rather than resurrecting it from disk", async () => {
    ...
    expect(Object.keys(state.get<Record<string, unknown>>("atlas.sessionMeta", {}))).toEqual(["doomed", "kept"]);
    ...
    await state.update("atlas.sessionMeta", { kept: { customName: "mine" } });
  ...
  it("atomically creates an install id and adopts the existing winner", () => {
    ...
    const firstId = first.getOrCreate("atlas.installId", () => "first-id");
    const secondId = second.getOrCreate("atlas.installId", () => "second-id");
  ...
  it("writes through a temp file and shadows into globalState", async () => {
    ...
    await state.update("atlas.repoPins", { "/repo": { cwd: "/repo", pinnedAt: 7 } });
    ...
    expect(memento.store.get("atlas.repoPins")).toEqual({ "/repo": { cwd: "/repo", pinnedAt: 7 } });
    expect(fs.dirs.has(DIR)).toBe(true);
  });

  it("serves reads from memory immediately after a write", async () => {
    const { state } = make();
    void state.update("atlas.repoArchives", { "/r": { cwd: "/r", at: 1, archived: true } });
    // Synchronous read, no await — this is how the 60+ call sites use it.
    expect(state.get<Record<string, { archived: boolean }>>("atlas.repoArchives", {})["/r"].archived).toBe(true);
  });
  ...
  it("falls back to globalState when the file is corrupt, without throwing", () => {
    const { state, logs } = make((f, m) => {
      ...
      m.store.set("atlas.sessionMeta", { s1: { customName: "survived" } });
    });
    expect(state.get<Record<string, { customName: string }>>("atlas.sessionMeta", {}).s1.customName).toBe("survived");
  ...
  it.each([
    ...
    const meta = state.get<Record<string, { customName: string }>>("atlas.sessionMeta", {});
  ...
  it("treats a non-string install id as unreadable", () => {
    const { state, memento, logs } = make((f, m) => {
      ...
      m.store.set("atlas.installId", "shadow-id");
    });
    expect(state.get<string>("atlas.installId")).toBe("shadow-id");
    expect(memento.store.get("atlas.installId")).toBe("shadow-id");
    expect(logs.join()).toMatch(/unreadable on disk/);
  });
  ...
  it("degrades to globalState when the disk cannot be written", async () => {
    ...
    fs.failWrites = true;
    await state.update("atlas.sessionMeta", { s1: { customName: "typed while read-only" } });
    // The shadow copy is the point: the value is not lost.
    expect(memento.store.get("atlas.sessionMeta")).toEqual({ s1: { customName: "typed while read-only" } });
    expect(state.get<Record<string, { customName: string }>>("atlas.sessionMeta", {}).s1.customName).toBe(
  ...
  it("returns the default and writes nothing when there is no prior state", async () => {
    const { state, fs } = make();
    expect(state.get("atlas.sessionMeta", {})).toEqual({});
    await state.flush();
    expect(fs.files.size).toBe(0);
  });
  ...
  it("keeps concurrent writes in order", async () => {
    const { state, fs } = make();
    void state.update("atlas.repoPins", { a: { cwd: "/a", pinnedAt: 1 } });
