...
beforeEach(() => {
  ...
  fixture.overrides = {};
  for (const [dir, name] of [[".atlas", "config.toml"], [".codex", "config.toml"], [".claude", "settings.json"]]) {
...
/** Overlay grok/Atlas home is ~/.atlas; Codex/Claude keep their own dirs. */
function providerConfigDir(provider: string): string {
  return provider === "grok" ? ".atlas" : `.${provider}`;
}
...
describe("provider config host dispatch", () => {
  ...
  it.each([["grok", GLOBAL_CONFIG_STUB], ["codex", ""], ["claude", "{}"]])("leaves a missing %s read untouched, then creates it on save", async (provider, stub) => {
    const sidebar = host();
    const dir = path.join(fixture.home, providerConfigDir(provider));
  ...
  it.each(["grok", "codex", "claude"])("refuses a %s config that appeared after the missing read", async (provider) => {
    const sidebar = host();
    const file = path.join(fixture.home, providerConfigDir(provider), provider === "claude" ? "settings.json" : "config.toml");
    fs.unlinkSync(file);
