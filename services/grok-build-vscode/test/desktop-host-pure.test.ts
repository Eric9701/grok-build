...
describe("desktop ConfigStore", () => {
  ...
  it("fires onDidChange for dotted keys", async () => {
    ...
    const seen: string[] = [];
    store.onDidChange((e) => {
      if (e.affectsConfiguration("atlas.cliPath")) seen.push("cliPath");
...
describe("createSafeStorageSecrets", () => {
  ...
  it("stores ciphertext, not the plaintext token", async () => {
    ...
    await secrets.store("atlas.remoteControl.deviceToken", token);
    const raw = fs.readFileSync(file, "utf8");
    expect(raw).not.toContain(token);
    expect(JSON.parse(raw).entries["atlas.remoteControl.deviceToken"]).toBeTruthy();
    expect(await secrets.get("atlas.remoteControl.deviceToken")).toBe(token);
  });
  ...
  it("unlink can delete ciphertext when get throws (offline kill-switch)", async () => {
    ...
    await createSafeStorageSecrets(file, xorStorage()).store(
      "atlas.remoteControl.deviceToken",
      "device-token",
    );
    ...
    await expect(broken.get("atlas.remoteControl.deviceToken")).rejects.toThrow(/decrypt/i);
    // delete does not need the OS key — this is the kill-switch property.
    await broken.delete("atlas.remoteControl.deviceToken");
    expect(await createSafeStorageSecrets(file, xorStorage()).get("atlas.remoteControl.deviceToken"))
      .toBeUndefined();
  ...
  it("mutation: a plaintext fallback would be detectable", async () => {
    ...
    let threw = false;
    try {
      await secrets.store("atlas.remoteControl.deviceToken", "plain-token-value");
    } catch (e) {
...
describe("desktop main wiring (source gates)", () => {
  ...
  it("desktop remoteSignOut uses the same native confirm as unlinkRemoteDevice", () => {
    ...
    expect(ext).toMatch(/registerCommand\("atlas.unlinkRemote", \(\) => sidebar\.unlinkRemoteDevice\(\)\)/);
    const unlinkStart = sidebar.indexOf("async unlinkRemoteDevice()");
    const unlinkEnd = sidebar.indexOf("private async postRemoteStatus", unlinkStart);
...
describe("encrypted voiceApiKey (A5)", () => {
  ...
  it("stores voiceApiKey only in the encrypted bag, never config.json", async () => {
    expect(SENSITIVE_CONFIG_KEYS.has("atlas.voiceApiKey")).toBe(true);
    ...
    expect(rawSens).toContain("atlas.voiceApiKey");
    expect(JSON.parse(rawSens).entries["atlas.voiceApiKey"]).toMatch(/^[A-Za-z0-9+/=]+$/);
  ...
  it("migrates legacy plaintext voiceApiKey out of config.json", async () => {
    fs.writeFileSync(
      configPath,
      JSON.stringify({
        config: { "atlas.voiceApiKey": "legacy-plain-key" },
      }),
      "utf8",
    );
    const store = new ConfigStore(configPath, new SensitiveConfigStore(sensPath, memSafe));
...
describe("voice-key migration never deletes unencryptable credential (round 12)", () => {
  it("production sequence leaves plaintext and throws when encryption is unavailable", () => {
    ...
    try {
      fs.writeFileSync(
        configPath,
        JSON.stringify({
          config: { "atlas.voiceApiKey": "legacy-plain-key", "atlas.cliPath": "/bin/x" },
        }),
        "utf8",
      );
  ...
  it("migrates on a later run once encryption becomes available", () => {
    ...
    try {
      fs.writeFileSync(
        configPath,
        JSON.stringify({ config: { "atlas.voiceApiKey": "legacy-plain-key" } }),
        "utf8",
      );
      const noEnc: SafeStorageLike = {
        isEncryptionAvailable: () => false,
...
describe("desktop provider global state", () => {
  it("preserves the shared provider keys without desktop-specific translation", async () => {
    ...
    try {
      ...
      await state.update("atlas.providerConnections", { grok: true, codex: true });
      await state.update("atlas.providerModelCache", { codex: { models: [{ modelId: "" }], seenAt: 1 } });
      await state.update("atlas.projectProviderDefaults", { "/work": { provider: "codex", modelId: "" } });

      const reloaded = createFileMemento(file);
      expect(reloaded.get("atlas.providerConnections")).toEqual({ grok: true, codex: true });
      expect(reloaded.get<any>("atlas.providerModelCache").codex.models[0].modelId).toBe("");
      expect(reloaded.get<any>("atlas.projectProviderDefaults")["/work"])
