...
describe("desktop Electron app (real window + fake CLI)", () => {
  ...
  beforeAll(async () => {
    ...
    fs.writeFileSync(
      configJson,
      JSON.stringify({ "atlas.cliPath": fixtureCli() }),
      "utf8",
    );
  ...
  it("rail group collapse state survives reload (localStorage)", async () => {
    ...
    const collapsed = await page.evaluate(() => {
      ...
      const key = "atlas.remote.railShape:default";
...
describe("desktop single-instance lock (real second launch)", () => {
  it("second launch with the same profile exits without a second window", async () => {
    ...
    fs.writeFileSync(cfg, JSON.stringify({ "atlas.cliPath": fixtureCli() }), "utf8");
  ...
  beforeAll(async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        config: { "atlas.cliPath": fixtureCli() },
      }, null, 2),
      "utf8",
    );
