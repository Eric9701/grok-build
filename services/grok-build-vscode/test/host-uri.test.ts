...
describe("URI identity at the Host boundary (remote-safe class fix)", () => {
  ...
  it("atlas.sendFile passes a portable Uri (fromVsCodeUri), not uri.fsPath", () => {
    const src = extension();
  ...
  it("atlas.sendFile attaches the Explorer's whole selection, not just the clicked file", () => {
    ...
    const src = extension();
    expect(src).toMatch(/"atlas\.sendFile"[\s\S]{0,900}uris\?:\s*vscode\.Uri\[\]/);
