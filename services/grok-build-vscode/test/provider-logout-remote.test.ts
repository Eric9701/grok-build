...
function makeSidebar(update: () => Promise<void> = async () => {}): any {
  ...
  sidebar.state = {
    get: vi.fn((key: string, fallback: unknown) => (key === "atlas.sessionMeta" ? sessionMeta : fallback)),
    update: vi.fn((key: string, value: unknown) => {
      if (key === "atlas.sessionMeta") sessionMeta = value as Record<string, unknown>;
      return update();
    }),
  };
  sidebar.host = {
...
describe("provider logout real-entry wiring", () => {
  it("provider logout command resets its remote tab and leaves the other provider untouched", async () => {
    ...
    expect(sent).toContainEqual({
      type: "error",
      text: "Atlas was signed out, so that conversation ended. This tab has been reset to a fresh session.",
    });
  ...
  it("provider logout command names a background conversation's draft without quoting it", async () => {
    ...
    expect(sidebar.state.get("atlas.sessionMeta", {})["background-grok"]).toMatchObject({
  ...
  it("other-provider reconnect restores a detached logical tab draft only when its composer returns", async () => {
    ...
    try {
      ...
      expect((sidebar.state.get("atlas.sessionMeta", {})["detached-grok"] as any).queuedDraft)
        .toBeUndefined();
    } finally {
      vi.useRealTimers();
    }
