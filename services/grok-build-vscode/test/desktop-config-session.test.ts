 ...
 * failure that produced this test: a throwaway `atlas.cliPath` pointing at a
...
describe("session config overrides (--config-json)", () => {
  ...
  it("takes effect for this run", () => {
    ...
    store.applySessionOverrides({ "atlas.cliPath": "/tmp/stub-grok" });
  ...
  it("never writes the override to disk, even as a file that exists", () => {
    const store = new ConfigStore(file);
    store.applySessionOverrides({ "atlas.cliPath": "/tmp/stub-grok", "atlas.showThinking": true });
  ...
  it("does not clobber a real value the user had already saved", async () => {
    ...
    overridden.applySessionOverrides({ "atlas.cliPath": "/tmp/stub-grok" });
  ...
  it("applyOverrides still persists — it is the deliberate path", async () => {
    const store = new ConfigStore(file);
    store.applyOverrides({ "atlas.showThinking": true });
