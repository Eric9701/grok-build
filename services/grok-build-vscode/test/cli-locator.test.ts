...
const FAKE_BIN_NAME = IS_WIN ? "atlas.cmd" : "atlas";
...
describe("locateGrokCli", () => {
  ...
  it("falls back to PATH when no config and no ~/.atlas/bin/atlas", () => {
    // Injectable env so HOME/PATH are not shared with other vitest threads.
    // Empty home means ~/.atlas/bin and ~/.grok/bin miss; PATH must find atlas.
    const found = locateGrokCli("", { PATH: tmpDir, HOME: tmpDir, USERPROFILE: tmpDir });
    expect(found?.toLowerCase()).toBe(fakeBin.toLowerCase());
  });

  it("finds atlas under ~/.atlas/bin before PATH", () => {
    const binDir = path.join(tmpDir, ".atlas", "bin");
    fs.mkdirSync(binDir, { recursive: true });
    const atlasName = IS_WIN ? "atlas.cmd" : "atlas";
    const atlasBin = path.join(binDir, atlasName);
    if (IS_WIN) fs.writeFileSync(atlasBin, "@echo atlas\r\n");
    else {
      fs.writeFileSync(atlasBin, "#!/bin/sh\necho atlas\n");
      fs.chmodSync(atlasBin, 0o755);
    }
    expect(locateGrokCli("", { PATH: "", HOME: tmpDir, USERPROFILE: tmpDir })).toBe(atlasBin);
  });
...
describe("cli version cache identity", () => {
  ...
  it("names the persisted globalState key", () => {
    expect(CLI_VERSION_CACHE_KEY).toBe("atlas.cliVersionCache");
  });
});
