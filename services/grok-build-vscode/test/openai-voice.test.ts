...
async function start() {
  ...
  const started = streamer.start({ apiKey: "secret", keyterms: ["atlas send"] });
...
describe("OpenAI Realtime lifecycle", () => {
  ...
  it("assembles cumulative deltas, replaces them with finals, and never sends from a partial", async () => {
    ...
    commit(ws, "a"); delta(ws, "a", "fix it "); delta(ws, "a", "atlas send");
    expect(events.at(-1)).toEqual({ text: "fix it atlas send", speechFinal: false });
    expect(sends).toEqual([]);
    final(ws, "a", "fix it Atlas said");
    expect(sends).toEqual([]);
    commit(ws, "b", "a"); final(ws, "b", "atlas send");
    expect(sends).toEqual(["fix it Atlas said atlas send"]);
    delta(ws, "b", "late stale delta");
    expect(s.transcript).toBe("fix it Atlas said atlas send");
    s.cancel();
  });
