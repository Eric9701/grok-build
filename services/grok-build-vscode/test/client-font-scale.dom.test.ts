...
describe("client font scale (remote)", () => {
  ...
  it("steps, resets, persists, and is bounded", () => {
    const { window, doc } = bootWebview({
      ...
      beforeScripts: (w) => {
        withRail(w);
        (w as any).localStorage.removeItem("atlas.remote.fontScale");
      },
    });
    ...
    expect((window as any).localStorage.getItem("atlas.remote.fontScale")).toBe("1.1");
    ...
    api.set(1);
    expect(api.get()).toBe(1);
    expect((window as any).localStorage.getItem("atlas.remote.fontScale")).toBe("1");
  });
...
describe("client font scale (desktop bridge)", () => {
  it("uses localStorage when grokDesktopShell is present and ignores host fontScale", () => {
    const { window, doc } = bootWebview({
      beforeScripts: (w) => {
        ...
        (w as any).localStorage.setItem("atlas.desktop.fontScale", "1.2");
      },
    });
    const api = (window as any).__grokFontScale;
    expect(api).toBeTruthy();
    expect(api.key).toBe("atlas.desktop.fontScale");
    ...
    api.set(1.5);
    expect((window as any).localStorage.getItem("atlas.desktop.fontScale")).toBe("1.5");
  });
  ...
  it("shows Text size on General and keeps the slider in sync with shortcuts", () => {
    ...
    expect(doc.body.style.getPropertyValue("--chat-zoom")).toBe("1.4");
    expect((window as any).localStorage.getItem("atlas.desktop.fontScale")).toBe("1.4");
  });
});
  ...
  it("does not wire client shortcuts or localStorage client key", () => {
    const { window, doc } = bootWebview({
      beforeScripts: (w) => {
        ...
        (w as any).localStorage.setItem("atlas.desktop.fontScale", "1.5");
        (w as any).localStorage.setItem("atlas.remote.fontScale", "1.5");
      },
    });
