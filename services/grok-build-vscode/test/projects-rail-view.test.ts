...
describe("projects rail view registration", () => {
  ...
  it("registers grok.projects in its own activity bar container", () => {
    ...
    expect(containers.some((c) => c.id === "atlasPrimary")).toBe(true);
    expect(containers.some((c) => c.id === "atlasProjects")).toBe(true);
    expect(PRIMARY_CONTAINER_ID).toBe("workbench.view.extension.atlasPrimary");
    expect(PROJECTS_CONTAINER_ID).toBe("workbench.view.extension.atlasProjects");

    const rail = pkg().contributes.views.atlasProjects;
    expect(rail).toBeDefined();
    expect(rail.find((v) => v.id === "atlas.projects")).toEqual({
      type: "webview",
      id: "atlas.projects",
      name: "Projects",
    });
    expect(GROK_PROJECTS_VIEW_ID).toBe("atlas.projects");

    // Chat stays in the secondary container — rail is alongside, not inside.
    const chat = pkg().contributes.views.atlasSidebar?.find((v) => v.id === "atlas.chat");
    expect(chat).toBeDefined();
    expect(rail.some((v) => v.id === "atlas.chat")).toBe(false);
  });

  it("keeps the rail out of every container chat can be moved into", () => {
    // The regression this exists for: the rail shipped inside `atlasPrimary`,
    ...
    for (const target of moveTargets) {
      ...
      expect(pkg().contributes.views[id]?.some((v) => v.id === "atlas.projects") ?? false).toBe(
        false,
      );
    }
  });
  ...
  it("gives the rail container a real icon that ships", () => {
    ...
    const containers = pkg().contributes.viewsContainers.activitybar;
    const railIcon = containers.find((c) => c.id === "atlasProjects")?.icon;
