...
function transcript(opts: { remote?: boolean; count?: number; height?: number; on?: boolean } = {}) {
  ...
  const h = bootWebview({
    remote,
    beforeScripts: (w) => {
      if (on && remote) (w as any).localStorage.setItem("atlas.remote.promptNav", "true");
    },
  });
  opened.push(h);
...
describe("prompt navigation (#150)", () => {
  it("is on for a remote that has never had an opinion, and stays off for one that said no", () => {
    ...
    expect(fresh.window.localStorage.getItem("atlas.remote.promptNav")).toBeNull();

    const refused = bootWebview({
      remote: true,
      beforeScripts: (w) => { (w as any).localStorage.setItem("atlas.remote.promptNav", "false"); },
    });
    opened.push(refused);
