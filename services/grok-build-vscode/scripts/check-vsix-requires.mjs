...
function packedFiles() {
  // Node 18: @vscode/vsce 3.9 pulls undici that needs global File (Node 20+).
  // Route through the same shim as `npm run package`.
  const shim = path.join(root, "scripts", "vsce-node18.cjs");
  const cmd = existsSync(shim)
    ? `node "${shim}" ls`
