 ...
 *
 * Keys match VS Code's dotted form (`atlas.cliPath`, …). `getConfiguration("grok")`
 * returns a section whose `.get("cliPath")` reads `atlas.cliPath`.
 *
 * Credentials (`atlas.voiceApiKey`) never live in plaintext config.json — they
...
export const CONFIG_DEFAULTS: Readonly<Record<string, unknown>> = {
  ...
  "atlas.voiceApiKey": "",
  "atlas.ffmpegPath": "",
  "atlas.voiceInputDevice": "",
  "atlas.voiceSendPhrase": "atlas send",
  "atlas.voiceKeyterms": [],
  "atlas.voiceLanguage": "",
...
export interface DesktopAppPrefs {
  ...
  /** Dotted config overrides (e.g. `atlas.cliPath`). */
...
export class ConfigStore {
  ...
  setWorkspaceRoot(root: string): void {
    ...
    this.save();
    this.fireChange("atlas.desktop.workspaceFolders");
  }
   ...
   * throwaway `atlas.cliPath` aimed at a stub agent survived every later launch,
  ...
  /**
   * @param section dotted-prefix section (`"grok"` → keys `atlas.*`)
  ...
  getConfiguration(section?: string, resourcePath?: string): HostConfiguration {
    ...
    const prefix =
      section === "grok" || section === "atlas" ? "atlas." : section ? section + "." : "";
    const store = this;
    return {
