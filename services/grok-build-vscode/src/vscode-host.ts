...
/** Read `atlas.*` first, then legacy `grok.*` when the Atlas key was never set. */
function wrapAtlasAwareConfiguration(resource?: vscode.Uri): HostConfiguration {
  const atlas = vscode.workspace.getConfiguration("atlas", resource);
  const grok = vscode.workspace.getConfiguration("grok", resource);
  const atlasHas = (key: string): boolean => {
    const inspected = atlas.inspect(key);
    return (
  ...
  return {
    get<T>(section: string, defaultValue?: T): T | undefined {
      if (atlasHas(section)) {
        return arguments.length >= 2 ? atlas.get<T>(section, defaultValue as T) : atlas.get<T>(section);
      }
      return arguments.length >= 2 ? grok.get<T>(section, defaultValue as T) : grok.get<T>(section);
    },
    update(section, value, target) {
      return atlas.update(section, value, toVsCodeTarget(target));
    },
    inspect<T>(section: string): ConfigInspect<T> | undefined {
      const raw = atlasHas(section) ? atlas.inspect<T>(section) : grok.inspect<T>(section);
...
export function createVsCodeHost(
  ...
  return {
    ...
    getConfiguration(section?: string, resourcePath?: string) {
      ...
      if (section === "grok" || section === "atlas") {
        return wrapAtlasAwareConfiguration(resource);
      }
    ...
    async linkRemote() {
      await vscode.commands.executeCommand("atlas.linkRemote");
    },
    async unlinkRemote() {
      await vscode.commands.executeCommand("atlas.unlinkRemote");
    },
