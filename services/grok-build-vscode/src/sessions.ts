...
/**
 * Resolve the Atlas/Grok home directory the way the CLI does
 * (`xai-grok-config::default_grok_home`):
 * 1. `$GROK_HOME` override
 * 2. `~/.atlas` if it exists
 * 3. else `~/.grok` if it exists (legacy)
 * 4. else `~/.atlas` (preferred default for new installs)
 *
 * Home is USERPROFILE on Windows and HOME elsewhere (matching the CLI's
 * `std::env::home_dir()` which ignores HOME on Windows).
 */
export function resolveGrokHome(
  ...
  const home = fromEnv || homedir();
  const atlas = path.join(home, ".atlas");
  const legacy = path.join(home, ".grok");
  try {
    if (existsSync(atlas)) return atlas;
    if (existsSync(legacy)) return legacy;
  } catch {
    // fall through to preferred default
  }
  return atlas;
}
