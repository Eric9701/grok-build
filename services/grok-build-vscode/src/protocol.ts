...
export type HostMsg =
  ...
  // atlas.soundNotifications — live toggle for the turn-complete/error sound (#59).
  | { type: "soundNotifications"; value: boolean }
  | { type: "processingSound"; value: boolean }
  // atlas.readRepliesAloud — local VS Code speech-synthesis preference.
  ...
  | { type: "atlasUpdateStatus"; current?: string | null; latest?: string | null; updateAvailable?: boolean; policy?: unknown; error?: string }
  ...
  // atlas.expandCommandOutputs — pre-expand every command's IN/OUT detail.
  | { type: "expandCommandOutputs"; value: boolean }
  // atlas.steerByDefault — send-while-busy skips the queue and steers (#52).
...
export type WebviewMsg =
  ...
  // atlas.soundNotifications gear switch (#59) — persisted globally by the host.
...
const HOST_MESSAGE_TYPE_MAP: Record<HostMsg["type"], true> = {
  initialState: true, planModeAvailability: true, showThinking: true, fontScale: true, atlasUpdateStatus: true,
...
const WEBVIEW_MESSAGE_TYPE_MAP: Record<WebviewMsg["type"], true> = {
  ...
  questionCancel: true, setModel: true, runInstallCmd: true, runAtlasLogin: true,
  logout: true, checkAtlasUpdate: true, updateAtlas: true, recheckConnection: true,
