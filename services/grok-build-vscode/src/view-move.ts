...
/** Projects rail — its own activity-bar container (`atlasProjects`). Separate from
 *  the chat view so opening it never creates a second chat client. */
export const GROK_PROJECTS_VIEW_ID = "atlas.projects";
...
export const GROK_CHAT_VIEW_ID = "atlas.chat";

/** Contributed containers, one per dock location (package.json prefixes each id
 *  with `workbench.view.extension.`). `atlasSidebar` homes the chat view;
 *  `atlasPrimary` and `atlasPanel` are empty by default and exist only as
 *  `vscode.moveViews` targets for chat; `atlasProjects` homes the rail. */
...
/**
 * The rail's own container — deliberately NOT `atlasPrimary`.
 *
 * The rail first shipped inside `atlasPrimary`, and that container is also the
 ...
 */
export const PROJECTS_CONTAINER_ID = "workbench.view.extension.atlasProjects";
...
export const VIEW_PLACEMENT_KEY = "atlas.viewPlacement";
 ...
 */
export const MOVE_VIEW_HINT_USED_KEY = "atlas.moveViewPickerUsed";

/** Where `createVsCodeHost` caches its capability probe. Named here because the
 *  first-run test below has to know about it. */
export const SECONDARY_SIDE_BAR_PROBE_KEY = "atlas.hostAcceptedSecondarySideBar";
 ...
 * view is dropped into Explorer, `workbench.view.extension.atlasSidebar` never
 * registers, and `atlas.open` fails with "command not found". The extension
 ...
 * rescued by an update. They get a chat that opens (`atlas.open` no longer
...
export function viewPlacementCorrection(opts: {
  ...
  //   moving -> workbench.view.extension.atlasPanel, panel right
  //   moved
  //
  ...
  // declared location. So `atlasPanel` and `atlasPrimary` land in the same place
...
/**
 * The command `atlas.open` should execute to reveal the chat.
 *
