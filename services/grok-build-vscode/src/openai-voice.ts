...
export function classifyOpenAiSttError(status: number): string {
  if (status === 401 || status === 403) return "OpenAI voice transcription was rejected (401/403). Set atlas.voiceOpenAiApiKey or OPENAI_API_KEY on the host to an OpenAI API-platform key. Codex / ChatGPT sign-in does not provide transcription API access.";
