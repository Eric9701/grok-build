...
This file is the complete spec for turning Atlas Desktop from an
...
mac:
  ...
  extendInfo:
    NSMicrophoneUsageDescription: >-
      Atlas Desktop uses the microphone for voice input in chat.
  target:
    # …unchanged
```
...
```bash
./scripts/verify-mac-signing.sh ~/Downloads/Atlas-Build-Desktop-<ver>-mac-arm64.dmg
```
...
codesign -dv --verbose=4 "/Applications/Atlas Desktop.app"
...
codesign --verify --deep --strict --verbose=2 "/Applications/Atlas Desktop.app"

# The one that actually predicts what a user sees
spctl -a -vvv -t install "/Applications/Atlas Desktop.app"
#   accepted
#   source=Notarized Developer ID

# Ticket is stapled, so first launch works with no network
xcrun stapler validate "/Applications/Atlas Desktop.app"
```
