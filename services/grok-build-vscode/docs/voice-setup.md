...
If you're signed in with **`atlas login`**, the extension reuses that stored token (`~/.atlas/auth.json`, with legacy `~/.grok/auth.json` fallback) for Speech-to-Text automatically. No separate key, nothing to paste.
...
| `atlas.voiceKeyterms` | `[]` | Words or phrases that help streaming recognition spell code and project vocabulary. The send phrase and `Atlas` come first; user terms fill the remaining xAI limit of 100 terms, 50 characters each. |
