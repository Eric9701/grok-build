...
The microphone button dictates speech using OpenAI Realtime transcription or SpaceXAI Speech-to-Text. Click it, wait for the listening waves, and speak — words appear live. Say **"atlas send"** to submit once the phrase is finalized and keep listening for the next message. Click the mic again to finalize the recording and keep the text.
...
If you are signed in with **`atlas login`**, voice **just works** — read the rest of
this section only if you need it.

### xAI — usually automatic

If you're signed in with **`atlas login`**, the extension reuses that stored token (`~/.atlas/auth.json`, legacy `~/.grok/auth.json`) for Speech-to-Text automatically. No separate key, nothing to paste.
...
Settings → Voice, set `atlas.voiceOpenAiApiKey`, or set `OPENAI_API_KEY` in the
...
|---|---|---|
| `atlas.voiceBackend` | `auto` | Credential-based routing above; `xai` or `openai` pins an explicit choice. |
| `atlas.voiceOpenAiApiKey` | `""` | OpenAI API key override; empty uses `OPENAI_API_KEY`. |
| `atlas.voiceOpenAiModel` | `gpt-live-transcribe` | Realtime model; also supports `gpt-4o-transcribe`, `gpt-4o-mini-transcribe`, and final-only `whisper-1`. |
| `atlas.voiceStreaming` | `true` | Live transcription. Disable for local one-shot batch mode after Stop; OpenAI batch uses `gpt-4o-transcribe`. Browser capture always streams. On xAI, batch costs $0.10/hr against streaming's $0.20/hr. |
...
| `atlas.voiceKeyterms` | `[]` | Words or phrases that help streaming recognition spell code and project vocabulary. The send phrase and `Grok` come first, up to 100 hints of 50 characters each. Each backend translates this list into its model's vocabulary format. |
| `atlas.voiceLanguage` | `""` | Recognition language hint; xAI also enables written formatting for numbers, currencies and units. Empty uses the backend default. |
